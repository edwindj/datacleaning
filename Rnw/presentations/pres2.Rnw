...
\begin{frame}
  ...
  \end{itemize}
  Rule checking straight in \R{}, but for set of rules this is problematic.
  Rules interact and it may be difficult to find the causing value.
\end{frame}

\section{Detection: \code{editrules}}
\begin{frame}
  \code{editrules} (de Jonge, van der Loo) is a toolbox to specify, maintain and check constraints/rules on data.
  It allows:
  \begin{itemize}
...
\begin{frame}[fragile]
\code{localizeErrors} look for the minimum number of values that should be changed.
<<tidy=FALSE>>=
E <- editset(c( "driverlicence %in% c(TRUE,FALSE)"
              , "if (driverlicence) age > 15"))
x <- data.frame(driverlicence=c(TRUE), age=c(12,18))
