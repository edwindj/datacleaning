...
statistical analysis.
    Data without values that obstruct analysis:
    \begin{itemize}
      \item Special values (\code{NA}, \code{NaN}, \code{Inf})
      \item Obvious errors, inconsistent values
    \end{itemize}
    \end{block}  
  \end{frame}
}
...
\begin{frame}
  \begin{itemize}
    \item {\em Detect} errors and special values
    \item {\em Remove} or {\em Correct} values.
    \item {\em Impute} missing values
  \end{itemize}
  
  \begin{block}{Note}
    \em All these actions have impact on your analysis. 
    They are data operations and should be scripted for reprodicability.
  \end{block}
\end{frame}
