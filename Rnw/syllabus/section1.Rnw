...
Most text books on statistics focus on modeling data and using these data 
in prediction or inferencing. They assume the data is in the correct state for 
data analysis.
In practice a data analyst spends most of his time on preparing the data before
doing any statistical operation.  It is very rare that data are in the correct format, are
without errors, are complete and have all the correct labels and codes that 
are needed for analysis. Data Cleaning is the process of transforming raw data
into consistent data that can be analyzed.

Data cleaning may sound as manipulating your data, which is
considered bad scientific practice.  However not cleaning your data makes
statistical analysis impossible. Incomplete and incorrect cleaning can 
heavily bias the results of your analysis.  
Data cleaning should therefore be seen as a statistical operation that
interprets raw data, removes known and obvious deficiencies and 
makes them technically correct
and consistent such that they are ready for further statistical analysis.

It is tempting and common practice to do manual data cleaning. 
However being a statistical operation data cleaning methods should be scripted, 
documented and reproducible: 
When applied
on the same raw data, the result should be the same. Scripting has the advantage
...
programming language all data cleaning steps can be executed in a scripted, 
...
data. Figure \ref{fig:steps} shows an overview of a typical data analysis
project. Here, each rectangle represents a certain          
...
\label{fig:steps}
\end{wrapfigure}
The first state (\textsl{Raw data}) is the data as it comes in. Often raw data
...
\textsl{Consistent data} is the stage where data is ready for
 statistical inference. It is the data that many statistics text
books use as a starting point. So ideally, this can be done without taking
previous data cleaning steps into account. 
However, data cleaning methods like imputation of missing values will 
influence statistical inference based on the consistent data.

Once your \textsl{Statistical results} are produced you can store them for
...
Elements of a vector can be selected or replaced using the square bracket operators
...
a two-index results in a vector. This behavior may be switched off using
