...
statistical inference while it is usually assumed that data is in the correct
...
For example, an \emph{age} variable may be reported negative, an under-aged person
...
point. Ideally, such theories can still be applied done without taking previous
...
towards the final result. Data cleaning is encompasses two of the five stages
...
\begin{center}
\begin{tabular}{ll}
  \code{numeric} & Numeric data (approximations of the real numbers, $\bs{R}$)\\
  \code{integer} & Integer data (whole numbers, $\bs{Z}$)\\
