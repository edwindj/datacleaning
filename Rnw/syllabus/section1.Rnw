...
or logicals.  In the case of logicals, the index is recycled if it is shorter
...
\code{data.frames} are a type of list, indexing them with a single index returs
...
a two-index results in a vector. This may be swithched off again using
