...
data are consistent with expectations 
\todo{Ik zou hier de woorden 'expectations based on' weglaten. Hoewel semantisch
en filosofisch
correct, roept expectations bij statistici een ander begrip op. Vooral omdat we 
hier harde fouten behandelen, lijkt het me handiger om hier een hardere formulering 
te kiezen. Misschien 'rules'/'constraints' i.p.v. expectations?} based on real-world 
knowledge about the
...
expectations are violated. For example, one \todo{Idem, ik heb hier de voorkeur
voor 'An age variable must be positive'} expects a positive value for an
\emph{age} variable.
...
example, \todo{'The marital status of a child must be unmarried'}
one expects that for persons under a certain age, the \emph{marital
status} is \emph{unmarried}. In the case of a violation it is not immediately
obvious whether \emph{age}, \emph{marital status} or both are wrong.
\item \emph{Correction} of the fields that are deemed erroneous by the
selection method.  This may be done through deterministic, deductive, or
model-based methods.
\end{enumerate}
%
...
recognize these substeps in order to make clear what assumptions are made
...
deterministic or deductive correction methods. In these techniques, erroneous
values are replaced by better ones by directly deriving them from other values
in the same record. Finally, subsection \ref{sect:mbimputation} gives an overview of some
commonly used imputation techniques that are available in \R{}.
...
perform statistical analyses on data where one or more values in the data are
...
with an empty field results in 1. This behaviour is most definitely unwanted
...
The behaviour of \R{}'s core functionality is completely consistent with the
...
account. A particularly easy-to-implement example is the method of Hiridoglou
and Berthelot\cite{hiridoglou:1986} for positive observations.  In this method,
...
seem available but it is implemented simple enough as follows.
<<tidy=FALSE>>=
...
\code{x} obey the rule `\code{x} must be nonnegative' one simply uses the following.
<<eval=FALSE>>=
x_nonnegative <- x >= 0
@
...
Moreover, since multivariate rules may be interconnected by sharing variables,
...
rules. Comments are written behind the usual \code{\#} character. The ruleset can
be read as follows.
<<cache=FALSE>>=
...
\label{sect:errloc}
The interconnectivity of edits is what make error localisation a hard problem.
...
Suppose that we alter age so that \code{num4} is not violated anymore. We then
run the risk of violating up to \emph{six} other edits.
...
developed to solve this minimisation problem\cite{waal:2011} of which two have
been implemented in \code{editrules}. The \code{localizeErrors} function provides
access to this functionality.

As an example we take two records from the \code{people} dataset from the previous subsection.
<<>>=
id <- c(2,5)
people[id,]
violatedEdits(E,people[id,])
@
We use a mixed-integer-programming approach to find the minimal
set of variables to alter. 
<<>>=
