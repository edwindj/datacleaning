...
statistical analysis. They are data in which missing values, 
special values, (obvious) errors and outliers are either removed, corrected 
or imputed.

Missing and special values obstruct most statistical methods, e.g. regression 
with incomplete data is cumbersome. Obvious errors and outliers heavily bias 
the outcome of most statistical estimates. Note that robust estimates can be 
seen as  a form of datacleaning: they typically remove outliers.

The process towards consistent data involves the following steps.
In  \emph{Detection} observations with missing values, special values, outliers
and obvious errors are identified. In \emph{Correction} values are changed or 
removed based on knowledge of 
  the observation. \emph{Imputation} fills the remaining missing values based on 
  knowledge of other observations or data.
  
A naive approach towards consistency is to stop after Detection and throw out all
problematic observations. However this strategy has severe problems. 
Firstly, throwing out partially missing observations also removes the available data 
for those observations. In surveys the partially missing observations are often 
the most interesting and different ones and throwing them out introduces an 
observation bias. Secondly many data sets have few complete observations. 
Removing incomplete data results in very few data to be analyzed reliably. 
...
into correction techniques that can repair obvious errors based
...
Below contains an outline of several methods for detecting missing values, special values, outliers 
and obvious errors in either individual variables or in combinations thereof.

\subsubsection{Missing values}
%<<echo=FALSE>>=
%person <- transform(person, height=as.numeric(as.character(height)))
%@
A missing value in an observation is either empty or unknown. 
In \R{} missing values are coded as \code{NA} and interpreted as unknown.
 
\begin{tip}{Tip}
Note that observed data with \code{NA} not always means ``unknown''. It some 
cases an empty value means $0$. This typical when using survey data or forms.
In that case you should explicitly impute zeros so this interpretation 
is recorded in the code. 
\end{tip}

\code{NA} obstructs many statistical functions in \R{}. 
<<>>=
age <- c(23, 16, NA)
...
mean(age, na.rm=TRUE)
@
With the parameter \code{na.rm=TRUE} most functions in R can remove the 
missing values, but this is cumbersome in analysis.

Which observations are complete can be
detected in \R{} with the function \code{complete.cases}.
This function returns a \code{logical} indicating which do not contain \code{NA}
values.  
<<>>=
print(person) 
complete.cases(person)
@
Using this result the incomplete rows can be removed from the \code{data.frame}.
In fact \R{} contains the \code{na.omit} function,
which does exactly that.
<<>>=
(persons_complete <- na.omit(person))
na.action(persons_complete)
@
The result of the \code{na.omit} function is a \code{data.frame} without 
incomplete rows, but also contains the names of the removed rows in the attribute 
\code{na.action}.

\subsubsection{Special values}
Numeric columns can have more formalized special values: \code{Inf}, \code{-Inf},
\code{NA} and \code{NaN} (see section \ref{sect:specval}).

Calculations involving special values result in special values, thereby 
obstructing statistical operations. Before analysis special values must 
be removed. 

\R{} contains several functions to deal with special 
values (e.g. section \ref{sect:specval}).
Checking for these values together can be done with 
\code{is.finite}.
<<>>=
is.finite(c(1, Inf, NaN, NA))
@
\code{is.finite} works on a vector. Using a utility 
function \code{is.special} a column of a \code{data.frame} can be checked for the
occurrence of special values
...
@
%
Observe that the \code{height} column is still stored as a \code{character} 
vector here
...
Outliers heavily influence the outcome of estimates. 
\begin{tip}{Note}
...
exponential or log-normal distribution for example. In that case one can either
...
with $x$ when it is larger than $x^*$. It is therefore appropriate for
...
\subsubsection{Obvious errors}
An obvious error is a value or combinations of values that cannot be true, based
on prior knowledge of the subject domain. For example, a
person's age cannot be negative, a man cannot be pregnant and a child cannot have
a drivers license.

This knowledge can be expressed in \emph{rules} or constraints. In data editing 
literature they are called \emph{edits}. A rule that must be obeyed by a record, is a 
{\em hard rule}. A rule that should be obeyed by a record, is a 
{\em soft rule}. Soft rules may indicate suspicious data.
Often data violates hard rules. In that is the case the following step is to 
find out which values are the cause. This is called error localization.
  
Rule implementation can be done straight in \R{}, but for a set of rules this is
problematic. Rules may interact and should be not seen as separate, 
but as a set of connected rules. Furthermore \R{} code quickly 
obscures the rules one tries to implement. 
It would be easier and more clear to define a rule set that all
...
The rules one needs to specify in \code{editrules} are valid \R{} syntax. They can 
...
xtable(person, caption="Person data, each observation contains at least one error.")
@
...
according to the rule specified: an \code{age} of \code{221} is not allowed. 
...
As can be seen from the table, the dataset contains more errors. Using 
a larger set of rules they can be detected.
...
variable is erroneous. For example in record 2 and 5 \code{child} and 
\code{married} are in conflict. Which values are correct?\footnote{Based on the 
other values of the record, this can be determined using error localization.}
<<width=7>>=
(ve <- violatedEdits(E, person[c(2,5),]))
@
\code{violatedEdits} shows that both violate rule \code{cat5}, but also other rules.
It is also possible to do some statistics on the violated edits. For large
datasets this can be very revealing: it may show problems in the collection of
the data. It may also show that some rules are never violated.  The result of
\code{violatedEdits} can be plotted.
%
\begin{wrapfigure}{r}{0.5\textwidth}
\begin{center}
<<>>=
...
for easy checking if observations contain valid data or not.
...
\label{sect:errloc}
For missing values, special values and univariate rules it is clear what values
are invalid. However for multiple multivariate rules, this is in general not  
clear. In the example of the previous section the variables \code{age},
...
Fellegi-Holt~\cite{Fellegi1976} principle which says: change the values 
of a minimal (weighted)
...
id <- c(2,5)
person[id,]
le <- localizeErrors(E, person[id,])
le$adapt
@
Note that although both records have conflicting values \code{child} and \code{married}
the solution is different (Why?).
...
Correction fixes an inconsistent observation by using the available data
of the observation. If correction is not possible the incorrect values are 
removed so they can be imputed.
...
one selects parts of the data, changes some variables, selects another part,
changes some more variables, etc. When such scripts are neatly written and
commented, they can almost be treated as a log of the actions performed
by the analyst. However, as scripts get longer it is better to 
...
this. Consider as an example the following (fictitious) dataset listing
...
In some cases a missing value can be determined because an observation and its 
hard rules generate a single solution. 
...
If after correction observations contain missing values, they have to be 
imputed with plausible values. Filling in missing data can be done using
advanced weight models or statistical estimation.
...
predictive mean matching, kNN: $k$-nearest-neighbors, int: interpolation,
