...
\begin{itemize}
  \item Detection: find observations with missing values, 
  special values, outliers and obvious errors.
  \item Correction: change or remove values based on knowledge of 
  the observation.
  \item Imputation: fill in missing values based on knowledge of other 
...
undesirable since it may introduce estimation bias or even leave you with to
...
outline several methods for detecting suspicious or erroneous in either
individual variables or in combinations thereof.

\subsubsection{Missing and special values}
%<<echo=FALSE>>=
...
With obvious errors are data that violate expectations based on
knowledge of the subject matter that the data describes. For example, a
person's age cannot be negative, or a person of 10 years old can not have
a drivers license. Rules ...
