...
This means that it should not contain obvious errors,
have special values such as \code{Inf} or \code{NaN}, that obstruct statistical
analysis. Also, suspicious values such as outliers should be identified, so they
...
\subsection{Consistent data in R}
