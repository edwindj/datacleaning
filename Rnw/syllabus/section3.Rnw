...
Observations containing \code{NA} can be
...
values. Using this result the complete rows can be selected from the 
\code{data.frame}.  
\R{} also contains the \code{na.omit} function,
...
\code{married} are in conflict. Which one is correct?\footnote{Based on the 
