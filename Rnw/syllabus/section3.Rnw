...
Such expectations are usually expressed in terms of rules wich may be interpreted as 
being obligatory (\emph{hard rules}), or indicative (\emph{soft rules}).

Observations (i.e. rows of a \code{data.frame}) that violate hard rules or have invalid values
can be left out of your analysis or in some cases be corrected or fill in (\emph{imputed}).
