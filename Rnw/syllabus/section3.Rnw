...
the subseqent statistical analyses.
...
undesirable since it may introduce estimation bias or even leavy you wiht to
...
\begin{subex}
  ...
  \item The variables in the file have to obey the following Rules:
    \begin{eqnarray*}
       \textrm{Species} &\in& \{\mcode{"setosa"}, \mcode{"versicolor"}, \mcode{"virginica"}\}\\
      \textrm{Sepal.Length} &>& 0\\
      \textrm{Sepal.Width}  &>& 0\\
      \textrm{Petal.Length} &>& 0\\
      \textrm{Petal.Width}  &>& 0\\
      \textrm{Species}      &>& 0\\
      \textrm{Sepal.Length} &<& 10\cdot\textrm{Sepal.Width}
    \end{eqnarray*}
