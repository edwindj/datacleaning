...
Checking with rules
  ...
  \item Find outliers in sepal length using \code{boxplot} and \code{boxplot.stats}. Retrieve the corrosponding observations and
   look at the other values. Any ideas what might have happened?
\end{subex}
\end{exercise}
...
Imputing
\begin{subex}
  \item Use sequential hotdeck imputation to impute \code{Petal.Width} by sorting the dataset on \code{Sepal.Length}.
  \item Use \code{kNN} imputation (\code{VIM}) to impute all missing values. Compare the imputed \code{Petal.Width} with the sequential hotdeck imputation method.
\end{subex}
\end{exercise}