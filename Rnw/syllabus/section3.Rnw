...
The process towards consistent data always involves the following 
three steps.
\begin{enumerate}
\item \emph{Detection} of an inconsistency. That is, one establishes which
constraints are violated. For example, an \emph{age} variable is constrained to
non-negative values.
...
@
The resulting \code{logical} can be used to remove incomplete records from the
...
indicated as separate points above or below the whiskers.
...
transformation.  Another option is to use a method that takes the skewness into
account. 
%
...
outliers on both sides of the distribution. Moreover, because of the
...
\subsubsection{Simple transformation rules}
...
in a \code{data.frame} which may be stored or analyzed. The rules that may
...
record-by-record. That is, it is not permitted to use functions like
...
the data. A \code{deducorrect} object also contains data on timing, user,
...
De Waal, Pannekoek and Scholtus\cite{waal:2011} (Chpt.\ 7) give a consice
overview of several well-established imputation methods.  
...
package has a function \code{lmImpute} but it uses the row number as auxiliary
...
category that most often occurs in the $k$ nearest neighbors is used. Both 
...
Checking with rules
  ...
  \item Determine how often each rule is broken (\code{violatedEdits}). Also summarize and plot the result.
  ...
  \item Find outliers in sepal length using \code{boxplot} and \code{boxplot.stats}. Retrieve the corrosponding observations and
   look at the other values. Any ideas what might have happened? Set the outliers to \code{NA} (or a value that you find more appropiate)
\end{subex}
\end{exercise}
