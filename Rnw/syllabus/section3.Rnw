...
data are consistent with expectations based on real-world knowledge about the
...
expectations are violated. For example, one may expect a positive value for an
\emph{age} variable.
...
example, one may expect that persons under a certain age, the \emph{marital
