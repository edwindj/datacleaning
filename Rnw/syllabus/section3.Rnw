...
and values that are erronous. After that we go into deterministic correction 
...
which returns a \code{logical} of which rows which have no \code{NA}.
The logical can be used to select a \code{data.frame}, but for convenience \R{}
contains the \code{na.omit} function.
<<>>=
person 
...
@
\code{is.finite} works on a vector. We can write a utility function \code{special}
that checks a column for the (non)occurence of special values
...
Rules may interact, so the rules should be not seen als separate rules, but as a
...
use these rules to check data for errors and find erronous values. It also allows
...
is violated, it is not clear which variable is erronous.
<<width=7>>=
...
\code{\$adapt} is a \code{logical} that signifies which variables are considired
erronous. It can be used in the correction and imputation procedures for filling in valid
