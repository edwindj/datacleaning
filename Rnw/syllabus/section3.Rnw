...
@
%
The behaviour of \code{localizeErrors} can be tuned with various options. It is
...
which is typically slower but has allows for more control. 
...
first, an error localisation method is used to empty certain fields, followed
...
combined with edit constraints rules force a unique solution.  As an example,
...
is goes beyond the scope of this tutorial to discuss all of them. In stead, we
...
implements a multiple imputation method based on the assumption of a
conditional multinormal variable distribution. The \code{deducorrect} package
