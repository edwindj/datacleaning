...
the outcome of most statistical estimates. Note that robust estimates are a 
form of datacleaning: they typically ignore outliers.
...
removed based on knowledge of 
  the observation. {\emph Imputation} fills the remaning missing values based on 
...
Removing incomplete data results in very few data to be analyzed reliabily. 
...
Calculations involving special values result special values. For analysis special
values should be removed. \R{} contains several functions to deal with special 
...
Outliers heavily influence the outcome of estimates. Note that outliers do not 
equal errors. Outliers should be detected, but not necessarily removed. Their 
...
person's age cannot be negative, a man cannot be pregnant and a child can not have
a drivers license.

This knowledge can be expressed in rules or constraints. In data editing 
literature they are called edits. A rule that must be obeyed by a record, is a 
...
problematic. Rules may interact, so the rules should be not seen as separate 
rules, but as a set of connected rules. Furthermore your \R{} code quickly 
obscures the rules you 
...
@
The data can be checked with \code{violatedEdits}. Record 4 contains an error according
...
are invalide. However for multiple multivariate rules, this is in general not  
...
In Correction one tries to fix an inconsistent observation by using the available data
of the observation. If not possible an incorrect value should be removed so it can
be imputed.
