...
\emph{Consistent data} is \emph{technically correct data} that is fit for statistical analysis.
This means that it should not contain errors, or have extraordinary values such as 
infinity or missing that obstruct the analysis.

The checking for glaring errors is based on the knowledge you
have based on subject matter. For example a persons age cannot be negative.
This knowledge is usually expressed in terms of rules wich may be interpreted as 
being obligatory (\emph{hard rules}), or indicative (\emph{soft rules}).

Observations (i.e. rows of a \code{data.frame}) that violate hard rules, have invalid values or are incomplete
may be left out of your analysis. This may seem to be the right procedure, but can introduce a bias or can result in
very few data to be analyzed. 
In many cases an invalid or imcomplete observation can be corrected and imputed, based on subject matter knowledge or 
the data of the other observations.

We will first 

\subsection{Checking for invalid values}
...
precise and/or robust against outliers than stanard (generalized) linear
...
Such methods are beyond th scope of the current text. The \code{imputation}
...
applied to numerical as wel as categorical data but is only viable when enough
...
shortly discuss four well-known flavours.

In \emph{random} hot-deck imputation, a value is chosen randomly, and uniformly
from the same data set. When meaningfull, random hot deck methods can be
...
In \emph{sequential} hot deck imputation, the vector conatining missings is
...
\emph{Predictive mean matching} (pmm) is a form of nearest-neighbour hot-deck
imputation whith a specific distance function. In predictive mean matching one
...
use pmm as a part of a more elaborata multilpe imputation scheme, which we
deemed out of scope for this tutorial.

\subsubsection{kNN-imputation}
In $k$ \emph{nearest neighbour} imputation one defines a distance
...
computed out of the $k$ nearest neighbours. In the case where a value is picked
from the $k$ nearest neighbours, kNN-imputation is a form of hot-deck imputation.

The \code{VIM} package contains a function called \code{kNN} that uses Gowers
distance\cite{gower:1971} to determine the $k$ nearest neighbours. Gower's
...
iris2 <- kNN(iris)
@
The \code{kNN} function determines the $k$ (default: 5) nearest neighbours of a
record with missing values. For numerical variables the median of the $k$
nearest neighbours is used as imputation value, for categorcal variables the
category that most often occurs in the $k$ nearest neighbours is used. Both the
...
arbitrary edit restrictions. One vialble option is therefore to minimally
