...
\R{} contains several functions to deal with special values. We encountered a
number of them in section \ref{sec:}.
 
\begin{tip}{Tip}
Note that sometimes a special value in your data may encode an 
...
The result of \code{is.special} shows which data have special values. Note that 
detecting them is good, but the data should be fixed (see \ref{sec:})
...
observation (or set of observations) that which appear to be inconsistent with
...
For more or less unimodal and symettrically distributed data, Tukey's box-and-whisker
...
\end{wrapfigure}
%
Of course, the box-and-whisker method can be visualized with the famous box-and-whisker
plot, where the box indicates the interquartile range and the median, the whiskers are
represented at the ends of the box-and-whisker plots and outliers are indicated with
an asteriks.

The box-and-whisker method fails when data are distributed skewly, as in exponential
or lognormal distributions for example. I that case one can either transform the data
for example with a logarithm, or square root transformation. Another option is to
use method that takes the skewness into account. A particularly easy-to-implement example
is the method of Hiridoglou and Berthelot\cite{hiridoglou:1986} for positive observations. 
...
\geq r,\textrm{ and } x>0.
\end{equation}
Here, $r$ is a usr-defined reference value and $x^*$ is usually the the median
...
<<width=7, fig.cap="Web of restrictions">>=
plot(E)
@
The result of \code{violatedEdits} shows
...
Felligi-Holt principle which says: change the values of a minimal (weighted) 
