...
outline several methods for detecting missing values, special values, outliers 
...
values. We have encountered several of them in section \ref{sect:specval}.
...
This knowledge can be expressed in rules or constraints. Data editing 
literature calls them edits. A rule that must be obeyed by a record, is a 
{\em hard rule}.
