...
(data <- read.csv("files//unnamed.txt"))
(data <- read.csv("files//unnamed.txt", header=FALSE))
colnames(data) <- c("age", "height")
print(data)
@

Without specifying \code{colClasses} \code{read.table} tries to detect the type of the column
automatically. This works quite will, but in case a numeric column contains a non numeric value
...
Note that since \code{data\$height} contains the value \code{5.9*}, \code{height} was read in as a character and coerced into a factor.
<<>>=
str(data)
summary(data)
@
...
\subsection{Recoding \code{factor}s and \code{character}s}
...
Categorical data is often stored as text values. If a categorical data column
is not neatly specified, we will need \code{character} manipulation (see \ref{sssect:charactermanipulation}).
...
Suppose in our dataset \code{gender} is encoded in \code{1} for female and \code{2} for male and \code{0} for 
unknown. With \code{factor} this can be recoded.
<<>>=
gender <- c(2,1,1,2,0,1,1)
#recode
(gender <- factor(gender, levels=c(2,1), labels=c("M","F")))
@
...
In some case you may want manipulate the order of the levels depending on another variable
This can be done with \code{reorder}
<<>>=
age <- c(72, 10, 23, 6, 67)
