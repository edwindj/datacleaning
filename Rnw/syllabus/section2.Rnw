...
3 & Split lines into separate fields    & \code{list}     \\
4 & Make every row of equal length      & \code{list}     \\
...
throwing out lines containing comments. You can use \code{grep} or \code{grepl}
for such selections (see section \ref{sect:appstringmatching}). 

\textbf{Step 3. Splitting lines into separate fields.} This can be done with
...
myfields <- strsplit(mychar,split=" ")
myfields
@
Here, \code{split} is a character of sequence of characters that are to be
...
\textbf{Step 4. Make every row of equal length.} 
...
structures like functions are objects endowed with a class label.
%
\begin{tip}{Tip}
...
\begin{subex}
  ...
  \item Coerce the \code{weight} column to \code{numeric}. 
\end{subex}
\end{exercise}
