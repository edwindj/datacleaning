...
With \emph{technically correct} data in \R{}, we mean that
\begin{itemize}
\item the data to be analyzed are stored in a \code{data.frame} with suitable columns names, and
...
\end{itemize}
%
When the data for your statistical analyses has been stored in a sufficiently
well-structured format (either binary/database or \code{text}/\code{XML}) one
of \R{}'s native import functions allows you to read the data into a correctly
typed \code{data.frame}. But even when raw data arrives as a neatly structured
\code{csv}-file you may have to take care that factor variables have the
appropriate levels.  

When you're less lucky, you'll get badly structured data and more work needs to
be done to transform the data into the desired format. Fortunately, \R{} has
many low-level tools available to finetune how data read into \R{} is to be
manipulated or interpreted.
%Data collected for statistical analysis usually come in {\em raw} format, which is typically not
%a \R{} data.frame. 
...
In this tutorial we assume that the data that needs to be read is in a text-format, which
already has several favorable properties over binary formats:
\begin{itemize}
\item It is a format that can be read by human beings. When you inspect a text-file, make sure
to use a text-reader (\code{more}, \code{less}) or editor (\code{Notepad}, \code{vim}) using
a fixed-width format. Do not use an office-application since their typesetting clutters the
data's structure (e.g. by replacing two characters by a ligature).
\item Text is very permissive in the types values that are stored, allowing for
comments and annotations.
\end{itemize}
%
Our task then, is to find ways to read a textfile into \R{} and have it
transformed to a well-typed \code{data.frame}. 
...
as a \code{csv} or \code{tab}-delimited file, getting data into a \code{data.frame} 
is fairly simple. One only needs to take care that exceptional values such
as \code{NA} or comment lines are interpreted correctly.

Things become more difficult when the storage format is not constant over the
file. For example, some numbers may have a period (\code{.}) as decimal
separator while others have a comma (\code{,}) and the number of columns may
vary accross lines. In such cases \R{}'s high-level data-reading functions such
as \code{read.table} are not always capable of reading the data correctly in a
single go.

Below we point out techniques to read normal and badly formatted text files
into \R{}. This tutorial is limited to rectangular data sets, although some of
the techniques mentioned here will be more broadly applicable.
...
the result of these functions is a \code{data.frame}. If your are lucky the
data is ``technically correct'' However, there are some tips and tricks we'll
point out here to help you read those almost-standard files.

\begin{tip}{Best practice}
Whenever you need to read data into \R{}, the resulting \code{data.frame} can quickly
be inspected with functions like \code{head}, \code{str}, and \code{summary}.
\end{tip}
...
\code{read}-functions mentioned earlier are just calls to \code{read.table}
with some arguments already fixed.  Specifically
\begin{center}
...
Each of these functions accept, amongst others, the following arguments.
%
\begin{center}
...
\hline
  ...
  \hline
  \code{sep}              & \code{""} &Field separator. Deafault is any sequence of whitespaces including \code{space}, \code{tab} and newline\\
  \code{header}           & \code{FALSE} & Does the first line contain column names?\\
  \code{na.string}        & \code{"NA"} & Which strings should be considered \code{NA}? \\
  \code{colClasses}       & \code{NA}   & \code{character} vector with the types of columns. Will coerce the columns to the specified types.\\
  \code{stringsAsFactors} & \code{TRUE}  & If \code{TRUE}, converts all \code{character} vectors into \code{factor} vectors.\\
\hline
\end{tabular}
\end{center}
Except for \code{read.table} each of the above functions assumes by 
default that the first line in the text file contains column headers,
as demonstrated in the following srcipt.
<<>>=
...
If \code{colClasses} is not specified by the user, \code{read.table} tries to
detect the type of the column automatically. This may yield unexpected results.
For example, in the above script, one of the rows contains a misformed
numerical variable (\code{5.9*}), causing \R{} to interpret the whole
column as a text variable. Moreover, by default text variables are converted
to \code{charater}, so we are now stuck with an \emph{height} variable expressed
as levels in a categorical variable:
<<>>=
str(person)
@
Using \code{colClasses}, we can force \R{} to either interpret the columns
in the way we want or throw an error.
<<tidy=FALSE>>=
read.csv("files/unnamed.txt",
  header=FALSE,
  colClasses=c('numeric','numeric'))
@
This behaviour may be desirable if you need to be strict about how
data is offered to your \R{} script. An added benefit is that
specifying \code{colClasses} significantly speeds up the reading process
since \R{} does not need to detect the column types experimentally.

A thirt option is to specify \code{stringsAsFactors=FALSE} so text
columns are read in as \code{character} vector, and conversion to
the desired format can be done within \R{}.
<<tidy=FALSE>>=
str( read.csv("files/unnamed.txt",
  header = FALSE,
...
%\begin{tip}{Best practice}
%t \code{as.factor} since \code{factor} give  control over 
...
\emph{classes} and \emph{types} in \R{}. For an example, see Exercise \ref{ex:factorconversion}. Below, we  introduce \R{}'s typing and
...
way for lower-level computer languages such as Clang{} to store approximations of real numbers. 
