...
If your data is stored in database or an other binary structured form, 
...
The \code{scan} function is a lower level text processing that allows you to read in blocks of formatted lines.
...
% this gave an error for some reason (mvdl)
%<<>>=
