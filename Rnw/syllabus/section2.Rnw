...
as a \code{csv} or \code{tab}-delimeted file you will in general have no
problems getting data into \R{} correctly. Things become more difficult however
when the storage format is not constant over the whole file. For example, in
...
In this section we point out techniques to read (badly) formatted text files
...
\end{tabular}
\end{center}
your data is basically in a ``technically correct'' state. However, there are
some tips and tricks we'll point out here to help you read those allmost-standard
files.
...
\end{tabular}
\end{center}
All these functions expect that the first line of the file contains column
headers by default.
%
%
%
...
By default, all \code{character} columns read by \code{read.table} etc. are
coerced to \code{factor} variables. This may be unwanted, for example when
a column contains uniqe identifying strings (like names) so converting to \code{factor}
is unwanted. This behaviour can be switched off completely either by specifying 
\code{stringsAsFactors=FALSE} or by explicitly tell \code{read.table} the 
class of each column with \code{colClasses}. Other arguments worth mentioning
...
yourself.  This is allows you to exercise precise control over how each line is
...
elements is concatenated with enough \code{NA}'s to reach lenght 3. 

The above code snippet does the job, but we can do a little better than that.
The \R{} function \code{lapply} offers a convinient way to loop over all
elemets in a list. \code{lapply} accepts as arguments a \code{list} and a
...
\begin{tip}{Tip}
Another advantage of \code{lapply} is that it is really easy to paralellize
...
stopCluster(cl)
@
Of course, paralellization only makes sense when you have a fairly long list to process,
...
Here we first copy all our elements in a \code{matrix} wich we'll then coerce
...
class specified behind the ``\code{as.}''. The only counterintuitive result
appears when coercing a \code{factor} or \code{ordered} variable to
\code{character}. The reason for this has to do with differences between
...
at the object. It is important to realize that \R{}'s coercion functions are fundamentaly
...
\code{A-Z}), arabic numbers (\code{0}-\code{9}), a number of interpunction
...
that extend \code{ASCII} or replace it alltogether. Some well-known schemes
...
\code{locale} settting.  This may fail when the file was not generated on the
...
\end{center}
You may copy the text from this pdf file in a textfile called
...
\begin{subex}
  \item Coerce the matrix to a \code{data.frame}, making sure all colums are
