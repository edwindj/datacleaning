...
You can find out the characer encoding of your system by typing (not
copy-pasting!) a non-\code{ASCII} character and ask for the encoding scheme,
like so.
<<eval=TRUE>>=
Encoding("Queensrÿche")
@
If the answer is \code{"unkown"}, this means that the local native encoding
is used. The default encoding used by your OS can be reqested by typing
<<>>=
...
sevice offered by the OS, or uses a third-party library included with \R{}.
