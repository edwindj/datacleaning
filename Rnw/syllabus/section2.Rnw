...
\begin{itemize}
\item The data to be analyzed are stored in a \code{data.frame}
\item Each column of the \code{data.frame} has the correct \R{} type.
\end{itemize}

This definition is simple, yet can be difficult in practice.
The data you collect for your statistical analysis in {\em raw} format, which is typically not
a \R{} data.frame. 

If your data is stored in a database or another binary structured form, retrieving
the data within R does give you a data.frame. However even in these cases it is advisable to
store your data in an open text format such as Comma Separated Values to make your analysis
reproducible. Furthermore, many times your imported data contains text values that need to be processed
to the right \R{} type: the columns of the data.frame need to have the right types.

Typically raw data are in a text file, which has several favorable properties:
\begin{itemize}
\item It is an open format that can be read by human beings
\item In many cases it is in a format that can be read by computer software
...
as a \code{csv} or \code{tab}-delimited file you will in general have no
...
some tips and tricks we'll point out here to help you read those almost-standard
files.
...
a column contains unique identifying strings (like names) so converting to \code{factor}
