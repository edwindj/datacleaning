...
interpreted and transformed into fields in a rectangular data set.

The following table indicates which steps need to be taken to go from a raw textfile
to a structured data set. We assume that every the data for a single row is contained
...
4 & Make every row equally long         & \code{list}     \\
