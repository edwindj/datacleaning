...
\begin{abstract}
Data cleaning, or data preparation is an essential part of a statistical
analyses. In fact, in practice it is often more time-consuming than the
statistical analyses itself. These lecture notes describes a range of
techniques, implemented in the \R{} statistical environment that allow the
reader to build data cleaning scripts for a wide range of data that is stored
in textual format. These notes cover technical as well as subject-matter
related aspects of data cleaning. Technical aspects include data reading, type
conversion and string matching and manipulation. Subject-matter related aspects
include topics like data checking, error localisation and an introduction to
imputation methods in R. References to relevant literature and \R{} packages
...
That is, we the reader is expected to be familiar with concepts such as
...
\textbf{Data.} Sometimes small data files are used as an example. These files
...
\textbf{References.} For brevity, referenes are numbered, occurring as superscript in
the main text.
